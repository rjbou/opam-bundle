This test verify bundling of real package `opam-ed.0.1` (old version) with compiler version 4.14.2 and opam version 2.0.
  ...
  $ opam-bundle opam-ed.0.1 --ocaml=4.14.2 --opam=2.0 --self --yes --archive-repo 2>&1 | sed 's/arch =.*/arch = $ARCH/;s/os =.*/os = $OS/;s/os-distribution =.*/os-distribution = $OSDISTRIB/;s/os-version =.*/os-version = $OSVERSION/;s/os-family =.*/os-family = $OSFAMILLY/'
